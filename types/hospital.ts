...
export interface Hospital {
   ...
   *
   * 세 상태(true/false/undefined)는 계속 구분해서 저장한다. 값은 반드시
   * 국민건강보험공단 검진기관 명단 대조나 병원 공식 홈페이지 공지로만 정하고,
   * 등급이나 다수결로 추정하지 않는다(데이터 입력 규칙 7번).
   *
   * 다만 카드에 배지로 보여 주는 것은 false일 때뿐이다 — 지정이 대다수라
   * 배지의 정보 가치가 낮아서다(39번 항목). 화면 표시 규칙이지 판정 규칙이 아니다.
   */
