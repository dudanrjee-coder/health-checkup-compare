...
const QUERIES = {
  ...
  // 서울현 36.4896724/127.2584015) 등록하기로 결정되면 검색어 없이도 바로 잡힌다.
  "sejong-trinium-womens-hospital": ["트리니움여성병원", "세종특별자치시 한누리대로 1934", "세종특별자치시 반곡동 한누리대로"],
  // 2026-09-24 대전광역시 tier=병원 2차 배치(HIRA 26~50번) 등록분.
  "daejeon-sungse-hospital": ["성세병원 대전", "대전광역시 유성구 온천북로33번길 21-32"],
  "daejeon-sintanjin-hanil-hospital": ["신탄진한일병원", "대전광역시 대덕구 신탄진로 623"],
  // OSM에 새손병원 노드가 없어 도로 기준 근사치다. 기본 조회는 계룡로의 용두동
  // 구간(34825)을 먼저 내주는데 공식 주소는 오류동이라, 오류동 구간(34857)으로 보정했다.
  "daejeon-saeson-hospital": ["새손병원", "대전광역시 중구 계룡로 858"],
  "daejeon-tantan-hospital": ["탄탄병원 대전", "대전광역시 서구 계룡로553번길 72"],
  ...
  // 2026-09-10 전북 tier=병원 2차 배치.
  "iksan-susarang-hospital": ["수사랑병원 익산", "전북특별자치도 익산시 배산로 69 (모현동1가)"],
  // "정다운병원"으로 조회하면 병원 건물이 아니라 문화동/옥산면 경계의 동명 버스정류장
  // (bus_stop)이 잡혀 실제 위치(나운동)에서 약 3km 벗어난다. 병원명 없이 법정동을 붙인
  // 도로명(나운동 대학로)만 써서 도로 단위 좌표를 쓴다.
  "gunsan-jeongdaun-hospital": "전라북도 군산시 나운동 대학로 236",
  "gunsan-mose-hospital": ["모세병원 군산", "전북특별자치도 군산시 문화로 47 (수송동)"],
  ...
  // 2026-09-16 재확인: 주소(오정구 포함)는 공식 홈페이지 오시는길 원문과 일치함이
  // 재검증됐으나, Nominatim에 이 건물 자체가 amenity=hospital 노드로 없다. 도로명
  // "소사로"·지번 "원종동 230-3"·영문 표기 모두 결과가 없거나 인근의 무관한
